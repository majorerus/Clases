...
$sql = "SELECT r.*, h.nombre AS hotelNombre FROM reservas r JOIN hoteles h ON r.idHotel = h.idHotel";
$result = $conn->query($sql);

// Eliminar reserva si se pasa el ID a través de GET
if (isset($_GET['eliminar'])) {
    $idReserva = $_GET['eliminar'];
    $sqlEliminar = "DELETE FROM reservas WHERE idReserva = $idReserva";
    if ($conn->query($sqlEliminar) === TRUE) {
        echo "Reserva eliminada correctamente.";
        header("Location: ver_reservas.php"); // Redirige a la misma página
    } else {
...
<div class="container mt-5">
        ...
        <a href="reservar.php" class="btn btn-success mb-3">Añadir Reserva</a>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?php echo $row['hotelNombre']; ?></td>
                        <td><?php echo $row['nombreCliente']; ?></td>
                        <td><?php echo $row['emailCliente']; ?></td>
                        <td><?php echo $row['fechaReserva']; ?></td>
                        <td><?php echo $row['fechaIngreso']; ?></td>
                        <td><?php echo $row['fechaSalida']; ?></td>
                        <td><?php echo $row['numeroPersonas']; ?></td>
                        <td>
                            <a href="editar_reserva.php?idReserva=<?php echo $row['idReserva']; ?>" class="btn btn-warning btn-sm">Editar</a>
                            <a href="?eliminar=<?php echo $row['idReserva']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de que deseas eliminar esta reserva?');">Eliminar</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
